...
"""
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
        with patch("yaml.safe_load", return_value={"hypon": {"user": "test_user", "password": "test_password"}, "mqtt": {"host": "test_host"}}) as mock_safe_load:
            config = load_config()
            mock_file.assert_called_once_with("config.yaml", "r", encoding="utf-8")
            mock_safe_load.assert_called_once_with(mock_file())
            assert config == {"hypon": {"user": "test_user", "password": "test_password"}, "mqtt": {"host": "test_host"}}
